...
const server=http.createServer(async (req,res)=>{
  ...
  if(p.startsWith("/api/")){
    ...
    // public player profile (by username)
    ...
    if(pm && req.method==="GET"){
      ...
      return json(res,200,{ player:{
        username:pu.username, name:pu.name, wins:pu.wins, earned, approved, totalSubs,
        posted: posted.length, rank: rank>=0?rank+1:null, banned:!!pu.banned, isAdmin:!!pu.isAdmin,
        joinedAt:pu.joinedAt, dares:posted } });
    }
