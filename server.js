...
const stats = ch => { const subs=db.submissions.filter(s=>s.chId===ch.id); const w=winnersOf(ch);
  ...
  if(p.startsWith("/api/")){
    ...
    if(p==="/api/me/activity"){
      ...
      const mySubs=db.submissions.filter(s=>s.userId===u.id).sort((a,b)=>b.at-a.at).map(s=>{
        ...
        return { id:s.id, code:ch&&ch.code, title:ch&&ch.title, file:s.file, hasVideo:!!s.video, status:s.status, reason:s.reason, at:s.at, won:!!won };
      });
    ...
    if(m && req.method==="GET"){
      ...
      view.mySubmission = db.submissions.find(s=>s.chId===ch.id&&s.userId===u.id) ? true : false;
    ...
    if(m && req.method==="POST"){
      ...
      save(); return json(res,200,{ ok:true, submissionId:id, hasVideo:!!videoName });
    }
    ...
    if(m && req.method==="POST"){
      ...
      sub.status="rejected"; sub.reason=reason; save(); return json(res,200,{ ok:true });
    }
